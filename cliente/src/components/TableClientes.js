...
const TableClientes = ({
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = clientes.map((n) => n.id_cliente);
  ...
  const removeCliente = (id_cliente) => {
    if (window.confirm("¿Estás seguro que quieres eliminar este usuario?")) {
      fetch(
        `${process.env.REACT_APP_SERVER_URL}/clientes/removecliente/${id_cliente}`,
        {
          method: "DELETE",
        }
      )
  ...
  const visibleRows = React.useMemo(
    ...
    [clientes, order, orderBy, page, rowsPerPage]
  );

  return (
    <Box sx={{ width: "98%" }} className="mx-auto">
